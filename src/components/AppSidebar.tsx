...
export function AppSidebar() {
    ...
    // Calculate updates for Clinic notification (Approved items active, or maybe just Approved to signal "Go ahead")
    // For now, let's notify about 'Approved' requests that likely need attention (e.g. sending device)
    // and 'Completed' requests that need confirmation/cost entry (if not yet entered)
    const clinicNotificationCount = currentUser?.clinicId
        ? requests.filter(r =>
            r.clinicId === currentUser.clinicId &&
            (r.status === 'Approved' || (r.status === 'Completed' && !r.repairCost)) // Approved or Completed but needing cost entry
        ).length
        : 0;
    ...
    const routes = isAdmin
        ? [
            ...
            },
            {
                ...
                badge: pendingCount > 0 ? pendingCount : undefined, // Add badge prop
            },
            {
        ...
        : [
            {
                ...
                href: "/clinic",
                color: "text-sky-500",
                badge: clinicNotificationCount > 0 ? clinicNotificationCount : undefined, // Add badge
            },
            {
            ...
            },
            {
                ...
                icon: Stethoscope,
                href: "/clinic/equipment", // View list of equipment
